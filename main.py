...
def create_message_list(messages):
  if not messages:
    return Div(P("No messages yet. Be the first to say hi!"),
               id="message-list",
               cls="empty-state")
  return Div(*[
      Div(Div(Div(Span(f"{msg['name']}: ", cls="message-name"),
                  Span(msg['message'], cls="message-text")),
              Small(msg['timestamp'], cls="message-timestamp"),
              cls="message-content"),
          cls="message") for msg in messages
  ],
             id="message-list",
             cls="message-list")
...
def create_guest_counter(count):
  return Div(f"{count} guests have said hi so far ",
             Span("👋", cls="emoji"),
             id="guest-counter",
             cls="guest-counter")
  ...
  form = Form(Div(
      ...
      Div(Label("Message:", fr="message"),
          Div(Input(
              ...
              oninput=
              f"updateCharCount('message', 'messageCount', {MAX_MESSAGE_CHAR})"
          ),
      ...
      cls="input-group"),
              ...
              hx_swap="innerHTML")
  footer = Div("made with ❤️ by ",
               A("matt", href='https://x.com/mattppal', target="_blank"),
               cls="footer")
  return (Title("matt's guestbook"),
          Main(
              H1("✍️ matt's guestbook"), form, H2("🪵 log"),
              Div(guest_counter,
                  message_list,
                  id="update-area",
                  hx_get="/update-messages",
                  hx_trigger="every 10s"), footer))
